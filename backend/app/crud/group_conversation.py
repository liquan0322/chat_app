...
class GroupConversationCRUD:
    ...
    async def create_group(db, creator_id, group_name):
        ...
        try:
            ...
            # 异步调用状态初始化
            await GroupConversationStateCRUD.init_group_state(db, group.id)
...
class GroupMessageCRUD:
    ...
    async def send_group_message(
        ...
        try:
            ...
            # 异步更新群组状态
            await GroupConversationStateCRUD.update_state_after_message(db, group_id, msg.id, is_human)
            return msg, "消息发送成功"
...
async def test_group_business():
    ...
    async for db in get_async_db():
        try:
            ...
            print(f"1. 创建群组：{msg}，群组ID：{group.id if group else '失败'}")

            # ------------------------------
            # 2. 添加成员到群组
            # ------------------------------
            member, msg = await GroupMemberCRUD.add_member_to_group(db, group.id, user_id=2)
            print(f"2. 添加成员：{msg}，成员ID：{member.id if member else '失败'}")

            # ------------------------------
            # 3. 添加机器人到群组
            # ------------------------------
            group_robot, msg = await GroupRobotCRUD.add_robot_to_group(db, group.id, robot_id=1)
            print(f"3. 添加机器人：{msg}，关联ID：{group_robot.id if group_robot else '失败'}")
            ...
            print(f"4. 发送人类消息：{msg}，消息ID：{human_msg.id}")
            ...
            print(f"   发送机器人消息：{msg}，消息ID：{robot_msg.id}")
            ...
            print(
                f"5. 群组状态：最后人类消息ID={state.last_human_message_id}，机器人连续回复数={state.consecutive_robot_replies}")

            # ------------------------------
            # 6. 查询群组所有消息
            # ------------------------------
            messages = await GroupMessageCRUD.get_group_messages(db, group.id)
            print(f"6. 群组消息列表（共{len(messages)}条）：")
            for msg in messages:
                sender_type = "人类" if msg.is_human else "机器人"
                sender_id = msg.sender_id if msg.is_human else msg.robot_id
                print(f"   [{sender_type}-{sender_id}]：{msg.message}")

            # ------------------------------
            # 7. 清理测试数据（删除群组）
            # ------------------------------
            is_del, msg = await GroupConversationCRUD.delete_group(db, group.id)
            print(f"7. 删除群组：{msg}，是否成功：{is_del}")
