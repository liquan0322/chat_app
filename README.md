...
后端框架：FastAPI=0.104.1，选择理由：高性能、自动生成 API 文档、类型提示、异步支持，适合构建现代化 API
编程语言：Python=3.11，选择理由：生态丰富、语法简洁，异步支持完善
数据库：PostgreSQL=15，选择理由：强大的关系型数据库，支持 JSON 类型（适合存储标签）、事务、索引优化
ORM：SQLAlchemy=2.0，选择理由：成熟的 ORM 框架，支持异步操作，类型安全
认证：JWT (PyJWT)=2.8.0，选择理由：无状态认证，适合前后端分离架构
前端：Vue 3 + Vite=4.4，选择理由：轻量、快速，易于实现简单交互界面
测试：Pytest=7.4，选择理由：Python 主流测试框架，支持异步测试
